...
import { Search, FileSpreadsheet, ChevronDown, ChevronUp, X } from 'lucide-react';
...
interface ResidentialContract {
  ...
  Date__c?: string | null;
  System_Size__c?: string | null;
  Contract_Price__c?: string | null;
  Install_Date__c?: string | null;
  CreatedDate?: string | null;
  [key: string]: any;
}

type SortField = 'Name' | 'Date__c' | 'Contract_Price__c';
type SortDirection = 'asc' | 'desc';

export function ResidentialContracts() {
  const [contracts, setContracts] = useState<ResidentialContract[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [sortField, setSortField] = useState<SortField>('Date__c');
  const [sortDirection, setSortDirection] = useState<SortDirection>('desc');
  const [selectedContract, setSelectedContract] = useState<ResidentialContract | null>(null);
  ...
  const loadContracts = async () => {
    try {
      ...
      let query = supabase
        .from('residential_contracts')
        .select('*', { count: 'exact' })
        .order('Date__c', { ascending: false, nullsFirst: false });

      if (searchTerm) {
        query = query.or(`Name.ilike.%${searchTerm}%,Contact__c.ilike.%${searchTerm}%,Stage__c.ilike.%${searchTerm}%`);
      }
      ...
      console.error('Error loading residential contracts:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredAndSortedContracts = useMemo(() => {
    return [...contracts].sort((a, b) => {
      ...
      if (sortField === 'Contract_Price__c') {
        aVal = parseFloat(aVal || '0');
        bVal = parseFloat(bVal || '0');
      } else if (sortField === 'Date__c') {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Residential Contracts</h1>
        <p className="text-slate-600">Manage solar installation contracts and agreements</p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th
                  onClick={() => handleSort('Date__c')}
                  className="text-left px-4 py-3 text-xs font-semibold text-slate-700 cursor-pointer hover:bg-slate-100 group"
                >
                  <div className="flex items-center gap-1">
                    Contract Date
                    <SortIcon field="Date__c" />
                  </div>
                </th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-700">System Size</th>
                <th
                  onClick={() => handleSort('Contract_Price__c')}
                  className="text-right px-4 py-3 text-xs font-semibold text-slate-700 cursor-pointer hover:bg-slate-100 group"
                >
                  <div className="flex items-center justify-end gap-1">
                    Contract Amount
                    <SortIcon field="Contract_Price__c" />
                  </div>
                </th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredAndSortedContracts.map((contract) => (
                ...
                >
                  <td className="px-4 py-3 text-sm font-medium text-slate-900">{contract.Name || '-'}</td>
                  <td className="px-4 py-3 text-sm text-slate-700">{contract.Contact__c || '-'}</td>
                  <td className="px-4 py-3">
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-emerald-100 text-emerald-700">
                      {contract.Stage__c || '-'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-sm text-slate-600">{formatDate(contract.Date__c)}</td>
                  <td className="px-4 py-3 text-sm text-slate-700">{contract.System_Size__c || '-'}</td>
                  <td className="px-4 py-3 text-right text-sm font-semibold text-slate-900">
                    {formatAmount(contract.Contract_Price__c)}
                  </td>
                  <td className="px-4 py-3 text-sm text-slate-600">{formatDate(contract.Install_Date__c)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {filteredAndSortedContracts.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-slate-600">No residential contracts found</p>
          </div>
        )}
      </div>

      {selectedContract && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={() => setSelectedContract(null)}>
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-slate-600 mt-1">Residential Contract Details</p>
              </div>
              <button
                      ...
                      <p className="text-sm text-slate-900">
                        {value === null || value === undefined || value === ''
                          ...
                          : key.toLowerCase().includes('date')
                            ...
                            : key.toLowerCase().includes('amount') || key.toLowerCase().includes('price') || key.toLowerCase().includes('cost')
