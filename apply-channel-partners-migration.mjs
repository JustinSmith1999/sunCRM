...
/*
  ...
  3. Security
    ...
    - Partner contacts can view their own partner data
    - Partner contacts can view leads for their partner
*/
...
-- Add partner_id to leads table if not exists
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Add partner_lead_source to leads table if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
async function applyMigration() {
  try {
    ...
    if (error) {
      ...
      console.log('Trying alternative approach with service role key...');
    ...
    console.log('Created tables: channel_partners, partner_contacts, partner_commissions');

  } catch (err) {
    console.error('Error:', err);
    console.log('\n📋 Please run this SQL manually in Supabase SQL Editor:');
    console.log('https://husbupeealwuxyopfwwb.supabase.co/project/_/sql/new');
    console.log('\n', migrationSQL);
  }
}
